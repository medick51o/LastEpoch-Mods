...
| WaypointManager | NOT a travel API — only tracks "waypoint use allowed in this zone" (`WaypointEnabled`, `EnableWaypoint()`). v1 force-enabled it on every click; v2 keeps the enable but ONLY immediately before a confirmed `LoadWaypointScene` (post-gate, post-lookup), so non-travelling clicks leave no footprint. Not independently verified as load-bearing — candidate for removal if WMW testing shows travel works without it |
