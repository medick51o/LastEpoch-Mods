...
namespace medick_CooldownTracker
{
    ...
    //
    // v5 rule: if there is no living player to anchor to, draw NOTHING.
    ...
    internal static class OverheadRenderer
    {
        ...
        public static void Draw()
        {
            ...
            SlotRegistry.SnapshotActive(_buf);
            if (_buf.Count == 0) return;
            if (!TryGetPlayerAnchor(cam, out Vector2 anchor)) return;
            ...
            for (int i = 0; i < _buf.Count; i++)
                DrawIcon(new Rect(startX + i * (sz + gap), startY, sz, sz), _buf[i], alpha);
        }
        ...
        static void DrawIcon(Rect r, SlotData s, float alpha)
        {
            ...
            GUI.DrawTexture(Styles.Pad(r, 3), Texture2D.whiteTexture);
            ...
            try
            {
                ...
                { Styles.DrawSprite(r, s.Icon); drewSprite = true; }
            }
            catch { }
            if (!drewSprite)
            {
            ...
            bool nearReady = s.Fill < 0.10f;
            Styles.DrawBorder(r, nearReady
            ...
            GUI.color = lblCol;
            GUI.Label(lr, s.DisplayLabel, Styles.IconLabel(fontSize, lblCol));
