...
namespace UnityEngine
{
    ...
    public class Transform : Object
    {
        ...
        public int ChildCalls;
        public T[] GetComponentsInChildren<T>(bool includeInactive)
        {
            ChildCalls++;
            if (FailChildren) throw new Exception("injected hierarchy failure");
            return (T[])(object)Object.Scene.Where(t => (t.transform == this || t.transform.IsChildOf(this)) &&
                (includeInactive || t.gameObject.activeInHierarchy)).ToArray();
        }
...
namespace Il2Cpp
{
    public class UITooltipItemAffix { public TextMeshProUGUI _text; }
    ...
    public class UITooltipItem : UnityEngine.Object
    {
        ...
        public GameObject content, compareContent, blessingContent, blessingCompareContent, resonanceContent;
        public TextMeshProUGUI implicitText, compareImplicitText, blessingImplicitTMP, blessingCompareImplicitTMP;
        public List<UITooltipItemAffix> uniqueAffixesText, prefixesText, suffixesText,
            compareUniqueAffixesText, comparePrefixes, compareSuffixes;
        public UITooltipItemAffix sealedAffix, sealedPrimordialAffix, sealedCorruptedAffix,
            compareSealedAffix, compareSealedPrimordialAffix, compareSealedCorruptedAffix;
...
internal static partial class Program
{
    ...
    {
        if (Prefs.Category == null) Prefs.Init();
        Prefs.DebugLog.Value = false;
        TooltipPerf.Tick();
        Call(typeof(FilterRuleTooltip), "ClearPending");
        Originals.Clear();
        Field<Dictionary<int, (UITooltipItem ui, object[] args)>>(typeof(TooltipRecolor), "s_tooltips").Clear();
        Set(typeof(TooltipRecolor), "s_lastFullSceneTime", -1f);
        Set(typeof(TooltipRecolor), "s_emptyBracketScans", 0);
        Set(typeof(TooltipRecolor), "s_affixHookWarned", false);
    ...
    private static void Main()
    {
        ...
        Check(ui.ChildScans == 31, "missing target: 31 discovery attempts at 60 FPS within 0.5 seconds despite repeated equal-content setters");
        ...
        Check(Field<long>(typeof(TooltipPerf), "s_ruleStart") == 1 &&
              ...
              Field<long>(typeof(TooltipPerf), "s_ruleNoTarget") == 31,
        ...
        foreach (string invalidation in new[] { "inactive", "detached", "renamed" })
        {
            ...
            Check(ui.ChildScans == 33 && ItemFilterManager.Instance.Filter.Calls == 1,
        ...
        Check(ui.ChildScans == 31, "ID-less equal-content wrappers retain retry bound");
        Capture(ui, new ItemDataUnpacked { id = null, subType = 8 }); FilterRuleTooltip.MonitorUpdate();
        Check(ui.ChildScans == 32, "ID-less changed material subtype starts a fresh generation");
        ...
        Check(ui.ChildScans == 31, "discovery exceptions are bounded too");
        ...
        Attach(ui.transform, empty, shard, marked, range);
        Prefs.DebugLog.Value = true;
        Scan(ui);
        ...
        Attach(ui.transform, stat);
        Scan(ui);
        Check(stat.text.Contains("<link=\"ttu\">") && stat.text.Contains("+2 Health Regen") && stat.text.EndsWith(Marker, StringComparison.Ordinal), "two-stat clean text retains border links and marker");
        ...
        ui.transform.FailChildren = true; UnityEngine.Object.FailScan = true; Scan(ui);
        ...
        Attach(ui.transform, stat, ground);
        Scan(ui);
        ...
        Check(Field<long>(typeof(TooltipPerf), "s_scans") == 3 &&
              Field<long>(typeof(TooltipPerf), "s_fullScene") == 0 && Field<long>(typeof(TooltipPerf), "s_scoped") == 3 && Field<long>(typeof(TooltipPerf), "s_tmps") == 3 &&
