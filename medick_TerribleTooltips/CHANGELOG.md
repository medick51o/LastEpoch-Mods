...
## v3.1.1-beta1 — shard-stutter diagnostic beta
...
- Caches rule matching and allows target discovery through 12 frames or 0.5 seconds (whichever expires first), then stops for that content. Repeated same-content setters do not restart the budget; late equipment rows within the window still receive Rule#. Invalidation of a previously found requirements row permits one bounded replacement-search window, without repeating matching; repeated setters with no replacement cannot restart it.
